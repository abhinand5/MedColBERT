...
Tier A: High-signal supervised retrieval data

- BioASQ training splits, excluding eval years/tasks.
- TREC Clinical Trials / Clinical Decision Support / Precision Medicine training
  material, excluding eval topics/corpus leakage.
- PubMed/PMC citation-context supervision:
  - query: citation sentence or surrounding paragraph.
  - positive: cited article title/abstract or cited PMC passage.
  - negatives: BM25, same-MeSH/same-topic non-cited papers, citation-neighborhood
    hard negatives.
- PMC case-report supervision:
  - patient summary or case context as query.
  - relevant article/case passage as positive.
  - similar disease/body-system cases as hard negatives.
- ClinicalTrials.gov supervision:
  - trial eligibility/condition/intervention text as document.
  - DSPy-generated patient vignettes as queries, filtered by exact condition and
    intervention constraints.

MedEmbed v1 is not a quality anchor. Its HF card shows a modest synthetic/constructed
dataset (~232k default triplets, ~27.6k corpus rows), and preview examples include
negatives that may be semantically close or ambiguous. Use it only as:

- a legacy baseline/continuity dataset,
- a small low-weight warm start,
- or an ablation showing that the new recipe improves over the old synthetic recipe.

Do not let MedEmbed dominate Stage 2 training.
...
Tier D: UMLS-grounded synthetic query-passage pairs
...
- Each example must pass deterministic filters and a teacher/reranker quality gate
  before entering the main training set.

This is the main leverage point of the project. Generic synthetic retrieval data is
variable because LLMs copy passage wording, ask generic questions, or drift away from
the evidence. UMLS-grounded synthetic data is different: the metathesaurus gives us
known target concepts, alternative surface forms, source vocabulary groups, and
nearby-but-distinct concepts for negatives. Treat this as the primary scaling source
once validation gates pass.

The intended recipe is:

1. Select a passage with high-confidence CUI annotations.
2. Select target CUIs and alternative terms from different vocabulary groups.
3. Ask DSPy to generate a realistic query for a role, using alternative vocabulary.
4. Reject examples that copy passage terms, miss target CUIs, or become too generic.
5. Attach concept-near and BM25/dense negatives.
6. Keep only examples with a clear teacher/reranker margin.

Tier E: Distillation and score supervision

- Use MedCPT cross-encoder, BioClinical cross-encoder, or another strong biomedical
  reranker to score candidate pools.
- Keep examples with a clear teacher margin between positive and negatives.
- Use teacher scores for distillation only after the hard-label triplet pipeline works.
...
- PubMed abstracts, especially when tied to BioASQ, citation context, MeSH, or qrels.
- PMC OA paragraphs and citation contexts.
- Clinical trial eligibility text and structured trial metadata.
- PMC case reports / PMC-Patients-style patient summaries, with eval split isolation.
- MedEmbed positives only as low-weight legacy data or ablation material.
...
- 2M-3M audited triplets.
- UMLS-grounded synthetic should be the largest bucket if the pilot validates it.
...
| Source | Target |
|---|---:|
| BioASQ/TREC/other qrel-backed retrieval | 150k-300k |
| PubMed/PMC citation-context pairs | 400k-900k |
| ClinicalTrials.gov UMLS-grounded patient vignettes | 150k-400k |
| UMLS concept warm-up pairs | 500k-1M |
| UMLS-grounded DSPy synthetic, teacher-filtered | 1M-2M |
| MedEmbed legacy data | 0-100k, low weight only |
| Dense-mined negatives | added after first model |

Training principle:

- Synthetic is not the problem; unconstrained synthetic is the problem.
- UMLS-grounded synthetic should be 40-55% of the Stage 2 training mix if audits and
  teacher margins are good.
- The v1 target should be 2M-3M audited examples, with UMLS-grounded synthetic as the
  main cross-vocabulary scaling source.
- Scale beyond 3M only if pilot ablations show UMLS-grounded synthetic improves eval
  metrics over unconstrained and no-synthetic variants.
...
For this project, DSPy is not merely a convenience layer. It is the mechanism for
turning UMLS into controlled synthetic supervision. The key comparison is not
"synthetic vs real"; it is:

1. no synthetic,
2. unconstrained synthetic,
3. passage-grounded synthetic,
4. UMLS-grounded synthetic,
5. UMLS-grounded plus teacher-filtered synthetic.

The fifth setting is expected to be the strongest. This ablation should be part of
the paper if the numbers support it.
